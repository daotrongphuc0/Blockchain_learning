...
export function internalRemoveTokenFromOwner(contract: NFTContract, accountId: string, tokenId: string) {
    ...
    tokenSet.remove(tokenId);   // xoa token
    if (tokenSet.isEmpty()) {
        contract.tokensPerOwner.remove(accountId);    // nếu k còn token thi xoa luon de giai phong bo nhớ va giam phí gas
    } else {
        contract.tokensPerOwner.set(accountId, tokenSet);    // update lại danh sach token
    }
}
