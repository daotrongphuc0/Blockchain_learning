...
export function internalAddSale({contract, token_id, price}: { contract: NFTContract, token_id: string, price: string }) {
    ...
    assert(senderId == token.owner_id, "Unauthorized");   //kiem tra ng gui co phai la owner k
    let sale = new Sale({
        ownerId: senderId,
        tokenId: token_id,
        price
...
export function internalOffer({contract, sale_id}: {contract: NFTContract, sale_id: string}) {
    ...
    assert(buyerId !== sale.owner_id, "you can't offer on your sale");    // k duoc mua cua minh
