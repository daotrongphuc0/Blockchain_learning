...
let NFTContract = (_dec = NearBindgen({}), _dec2 = initialize({
  ...
  // nguoi so huu
  // danh sach token cua owner id
  //  id cua token

  // thong tin bo sung cho Nft vd mausac kich thuoc
  ...
  nft_transfer({
    ...
    approval_id,
    memo
  }) {// memo la ghi chu them de sau nay doc lai hieu mh da lam gi
  }
  nft_transfer_call({
    receiver_id,
    token_id,
...
function nft_min() {
  ...
  let _args = NFTContract._getArgs();
  let _result = _contract.nft_min(_args);
  NFTContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(NFTContract._serialize(_result));
}
function init() {
...
export { NFTContract, NFT_METADATA_SPEC, NFT_STANDARD_NAME, init, nft_metadata, nft_min, nft_resolve_transfer, nft_supply_for_owner, nft_token, nft_tokens, nft_tokens_for_owner, nft_total_supply, nft_transfer, nft_transfer_call };
